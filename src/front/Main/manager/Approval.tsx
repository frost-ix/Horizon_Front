...
const Approval = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3100/approval/users", {
        headers: {
  ...
  // 리스트 데이터
  // const data = [
  //   { check: 0, userId: 'hoseo123', name: '김호서', studentId: '2025201012', department: 'ICT융합보안', phone: '010-1234-5678', email: 'hoseo123@naver.com', photo: 'test1.jpg'},
  //   { check: 1, userId: 'nohappy34', name: '노행복', studentId: '2025201031', department: '호텔조리', phone: '010-1111-2222', email: 'nohappy34@gmail.com', photo: 'test2.png'},
  //   { check: 2, userId: 'silverstar1101', name: '이은별', studentId: '2025202015', department: '웹툰애니메이션', phone: '010-1101-1101', email: 'silverstar1101@naver.com', photo: 'test3.jpg'}
  // ];
  ...
  const handleSingleCheck = (checked : boolean, check : string) => {
    if (checked) {
  ...
  const handleAllCheck = (checked : boolean) => {
    if(checked) {
      ...
      const idArray : any = [];
      data.forEach((el) => idArray.push(el.userId));
    ...
    if (window.confirm("회원가입 승인 하시겠습니까?")) {
      ...
      if(checkItems){
        checkItems.forEach((checked) => {
          const account = data.find(el => el.userId === checked);
      ...
      <div className='list'>
        ...
        <table className='board-table'>
            ...
            <tr>
                ...
                <th className='th-department'>학과</th>
            ...
            {data.map((item, index) => (
              <tr key={index}>
                <td><input type='checkbox' id={`check-${index}`} name={`select-${item.userId}`}
                  onChange={(e) => handleSingleCheck(e.target.checked, item.userId)}
                  // 체크된 아이템 배열에 해당 아이템이 있을 경우 선택 활성화, 아닐 시 해제
                  checked={checkItems.includes(item.userId)} /></td>
                <td>{item.userId}</td>
                <td>{item.name}</td>
